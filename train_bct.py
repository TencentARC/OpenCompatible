...
def main_worker(device, ngpus_per_node, args, config):
    ...
    # build solver
    optimizer = torch.optim.SGD(model.new_model.parameters(), lr=args.optimizer["lr"],
    ...
    if args.new_model["resume"] is not None:
        best_acc1 = resume_checkpoint(model.new_model, optimizer, grad_scaler, args, logger)
    ...
    # build classification loss and backward-compatible loss
    criterion = {}
    criterion['base'] = nn.CrossEntropyLoss().cuda()
    if args.comp_loss["type"] in ["bct", "bct_ract"]:
        criterion['back_comp'] = nn.CrossEntropyLoss().cuda()
    elif args.comp_loss["type"] in ["contra", "triplet", "l2", "contra_ract", "triplet_ract"]:
        criterion['back_comp'] = BackwardCompatibleLoss(temp=args.comp_loss["temperature"],
                                                        margin=args.comp_loss["triplet_margin"],
                                                        topk_neg=args.comp_loss["topk_neg"],
